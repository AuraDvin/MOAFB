...
const APEX_SPEED_BONUS :float = 1.2
...
export var max_fall_speed: float # test 
...
func _ready() -> void:
	print(jump_buffer_clear_timer.connect("timeout", self, "clear_jump_buffer"))
...
func _physics_process(delta: float):
	...
	hinput = Input.get_axis("ui_left", "ui_right")
	
	print(hinput , " " , $".".scale.x)
	if (hinput < 0 && $".".scale.x > 0):
		$".".scale.x = - $".".scale.x
	elif ($".".scale.x < 0 && hinput > 0):
		$".".scale.x = - $".".scale.x
	...
	_is_at_jump_apex = Input.is_action_pressed("ui_accept") and _did_jump and abs(velocity.y) <= 96.0
	...
	if Input.is_action_just_released("ui_accept") and _did_jump and velocity.y < 0.0:
